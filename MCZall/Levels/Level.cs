    ...
    public class Level
    {
        ...
        retryTag4: try { CreateTable.ExecuteNonQuery(); } catch (Exception e) { totalCount++; if (totalCount < 10) goto retryTag4; else { Server.ErrorLog(e); return null; } }
            ...
            if (File.Exists(path))
            {
                            ...
                            try
                            {
                                if (line[0] != '#')
                                {
                                    switch (line.Substring(0, line.IndexOf(" = ")).ToLower())
                                    {
                                        ...
                                        case "animal ai": level.ai = bool.Parse(line.Substring(line.IndexOf(" = ") + 3)); break;
                                        case "edge water": level.edgeWater = bool.Parse(line.Substring(line.IndexOf(" = ") + 3)); break;
                                        case "survival death": level.Death = bool.Parse(line.Substring(line.IndexOf(" = ") + 3)); break;
                                        case "fall": level.fall = int.Parse(line.Substring(line.IndexOf(" = ") + 3)); break;
                                        case "drown": level.drown = int.Parse(line.Substring(line.IndexOf(" = ") + 3)); break;
                                        case "motd": level.motd = line.Substring(line.IndexOf(" = ") + 3); break;
                                        case "jailx": level.jailx = ushort.Parse(line.Substring(line.IndexOf(" = ") + 3)); break;
                                        case "jaily": level.jaily = ushort.Parse(line.Substring(line.IndexOf(" = ") + 3)); break;
                                        case "jailz": level.jailz = ushort.Parse(line.Substring(line.IndexOf(" = ") + 3)); break;
                ...
                catch (Exception ex) { Server.s.Log("ERROR loading level \"" + name + "\"."); Server.s.Log(ex.StackTrace); return null; }
                finally { fs.Close(); }
            }
